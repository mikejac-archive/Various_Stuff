...
JParser_parse(JParser* o, const U8* buf, U32 size)
{
   ...
   {
      DTXT("JParser_parse(): o->status != JParsStat_Done\n");
      baAssert(o->status == JParsStat_ParseErr);
      baAssert(o->status == JParsStat_MemErr);
      baAssert(o->status == JParsStat_IntfErr);
      /*baAssert(o->status == JParsStat_ParseErr ||
...
L_endParse:
            ...
            //DTXT("JParser_parse(): 6\n");
            break;

         default:
            //DTXT("JParser_parse(): default\n");
            baAssert(0);
      } /* end switch */
   } /* end for(;;) */
}
